...
// TODO_I: decide what to do with this
import { getObservedAppState } from "../../excalidraw/store";
...
export class ElementsDelta implements DeltaContainer<SceneElementsMap> {
  ...
  // TODO_I: does it make sense having a separate snapshot?
  public applyTo(
    elements: SceneElementsMap,
    ...
    (id: string, partial: ElementPartial) => {
      ...
      if (!element) {
        ...
        if (element) {
          ...
          // TODO_I: the version itself might be different!
   ...
   *
   * // TODO_I: revisit since arrows seem to be often redrawn incorrectly
   *
  ...
  private static stripIrrelevantProps(
    ...
    // TODO_I: we might need to strip the seed as well
